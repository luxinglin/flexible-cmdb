...
>
    ...
    >
    ...
    )
        {
            ...
            if (dx > 0) {
                ...
                item = (() = > {
                    let result1 = []
                    for(let i = rank.values.length - 2;
                i >= 0;
                i--
            )
                {
    ...
    )
        {
                ...
                for(operator of Array.from(rank.values))
            {
                ...
                if (operator.children.length) {
                    let x = d3.sum(operator.children, child = > linkWidth(child) * center(child)
                ) /
    ...
    >
        ...
        width = (d3.max(operators, o = > o.x) - d3.min(operators, o = > o.x
    ))
        +operatorWidth
    ...
    let render = function (operators, links, width, height, redisplay) {
        ...
        svg.transition()
            ...
            .attr('viewBox', [d3.min(operators, o = > o.x) - margin, -margin - height, width + (margin * 2), height + (margin * 2)
    ].
        join(' ')
    )
